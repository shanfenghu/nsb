...
def lookup_z_true(founder_counts_path: Path, n_test_max: int) -> tuple:
    """
    Looks up the ground truth founder count (z_true) for the cluster with size n_test_max.
    
    This function reads the founder counts CSV file (generated by extract_founder_counts.r)
    and finds the outbreak with matching cluster size to retrieve the actual number of
    founders (patient zeros) recorded in the dataset.
    
    Args:
        founder_counts_path: Path to CSV with columns: outbreak_id, cluster_size_n, founder_count_z
        n_test_max: Cluster size to look up (must match exactly)
        
    Returns:
        tuple: (z_true, outbreak_id) if found, (None, None) if not found
    """
    try:
...
def compute_kl_divergence(P: np.ndarray, Q: np.ndarray) -> float:
    """
    Computes Kullback-Leibler divergence D_KL(P || Q) to quantify information gain.
    
    In Bayesian inference, D_KL(Posterior || Prior) measures the "surprise" or the
    amount of information the likelihood has added to our initial assumptions.
    Higher KL divergence indicates more information gain from the observations.
    
    Args:
        P: Probability distribution (posterior) - must be non-negative
        Q: Reference probability distribution (prior) - must be non-negative
        
    Returns:
        KL divergence value (non-negative, zero if P == Q)
    """
    eps = 1e-10
...
def fit_prior_parameters(founder_counts_path: Path, train_data: np.ndarray, 
    ...
    """
    Fits prior distribution parameters from founder count data on the training set.
    
    This function extracts founder counts (z values) from the training set and fits:
    - Poisson prior: lambda = mean(z) (method of moments)
    - Negative Binomial prior: r and p via method of moments using mean and variance
    
    The same train/test split is applied to founder data to ensure consistency with
    the cluster size data split.
    
    Args:
        founder_counts_path: Path to CSV with columns: outbreak_id, cluster_size_n, founder_count_z
        train_data: Training cluster sizes (n values) - used to ensure consistent split
        test_size: Test split ratio (must match the split used for cluster sizes)
        seed: Random seed for reproducibility (must match the seed used for cluster sizes)
        
    Returns:
        dict with keys:
            - 'poisson_lambda': Mean of Poisson prior
            - 'negbin_r': Shape parameter of Negative Binomial prior
            - 'negbin_p': Success probability of Negative Binomial prior
    """
    try:
        df_founders = pd.read_csv(founder_counts_path)
...
def get_poisson_p_dist(r0: float, k_max: int) -> torch.Tensor:
    ...
    The Poisson distribution serves as a baseline for comparison, representing
    a simple branching process without overdispersion. For Poisson, R0 = lambda.
...
def get_nb_p_dist(r0: float, k_max: int, overdispersion: float = 0.1) -> torch.Tensor:
    ...
    The Negative Binomial distribution captures overdispersion (variance > mean),
    making it a better baseline for superspreading events than Poisson. The overdispersion
    parameter controls the tail heaviness: smaller values = more overdispersed.
    
    Args:
        r0: Target reproductive number (mean of distribution)
        k_max: Maximum offspring count (truncation depth)
        overdispersion: Dispersion parameter (smaller = more overdispersed, default=0.1)
...
def scale_offspring_distribution(p_dist: torch.Tensor, scale_factor: float) -> torch.Tensor:
    ...
    if r0_target > 0 and r0_target != r0_current:
        ...
        # Binary search for tilting parameter λ
        lambda_low = -2.0
        lambda_high = 2.0
        max_iter = 30
        tolerance = 1e-4
...
def run_benchmarks(n_range, k_depth, p_dist):
    """
    Measures NSB runtime and projects theoretical baseline complexities.
    
    Benchmarks the NSB spectral engine's actual runtime and projects theoretical
    baselines for comparison:
    - Tree Search: O(4^n) exponential complexity
    - Direct Convolution: O((nK)^2) quadratic complexity
    - NSB Spectral: O(nK log(nK)) near-linear complexity (measured)
    
    Args:
        n_range: Array of cluster sizes to benchmark
        k_depth: Truncation depth K (distribution length)
        p_dist: Offspring distribution to use for benchmarking
        
    Returns:
        tuple: (nsb_times, tree_search_times, direct_conv_times) as numpy arrays
    """
    nsb_times = []
    ...
    # Tree Search: O(4^n) exponential complexity, normalized to first n
    n0 = n_range[0]
    tree_search = [nsb_times[0] * (4**(n - n0)) for n in n_range]
    # Direct Convolution: O((nK)^2) quadratic complexity, normalized to first n
    direct_conv = [nsb_times[0] * ((n / n0)**2) for n in n_range]
    
    return np.array(nsb_times), np.array(tree_search), np.array(direct_conv)
...
def plot_duality_dashboard(p_nsb: torch.Tensor, n_test_max: int, z_true: int, 
    ...
    Args:
        ...
        prior_params: Dictionary with 'poisson_lambda', 'negbin_r', 'negbin_p' for priors
        outbreak_id: Optional outbreak identifier for annotations
        
    Saves:
        PDF figure to figures/exp_task_who.pdf
    """
    setup_plot_style()
    ...
    # Performs out-of-sample source attribution on the largest unseen cluster from the
    # SARS/MERS test set. Shows posterior distributions P(z|n) under three prior assumptions:
    # (1) Flat: uniform risk of seeding, (2) Poisson: sparse independent seeding,
    # (3) NegBin: clustered/burst seeding. The gradient background shows the raw likelihood
    # P(C=n|Z=z;θ), and insets show KL divergence (information gain) and R0 sensitivity.
    ...
    # Visualizes the internal mathematical representation of the learned transmission law
    # by evaluating the PGF G(s) = sum_k p_k * s^k on the unit circle s = exp(i*θ).
    # The NSB curve shows overdispersion (superspreading) compared to Poisson baseline,
    # with the "Overdispersion Gap" shaded area quantifying the difference. The curve
    # is colored by phase angle θ using a gradient colormap.
    ...
    # Demonstrates the O(nK log(nK)) scaling of the NSB spectral engine vs. exponential
    # (tree search) and quadratic (direct convolution) baselines. This proves the method
    # prevents the "Curse of Dimensionality" and enables real-time source attribution
    # even for very large clusters (up to n=10^6). The exponential and quadratic curves
    # are clipped as they cross into the "Computationally Prohibitive" region.
